...
import MillenniumAxiosClient, {
  getCurrentServerTime,
} from "./millennium_axios_client";

export const SUBMIT_TRANSACTION_TIMEOUT = 60 * 1000;

const SECTORS_IN_DELO = 10000000;
...
function _getAmountInDelos(amt: BigNumber) {
  return new BigNumber(amt).div(SECTORS_IN_DELO).toString();
}

/**
 * Server handles the network connection to a [Millennium](https://www.aiblocks.io/developers/millennium/reference/)
 * instance and exposes an interface for requests to that instance.
 * @constructor
 * @param {string} serverURL Millennium Server URL (ex. `https://millennium-testnet.aiblocks.io`).
...
export class Server {
  /**
   * serverURL Millennium Server URL (ex. `https://millennium-testnet.aiblocks.io`).
   *
  ...
  constructor(serverURL: string, opts: Server.Options = {}) {
    ...
    if (!isEmpty(customHeaders)) {
      MillenniumAxiosClient.interceptors.request.use((config) => {
    ...
    if (this.serverURL.protocol() !== "https" && !allowHttp) {
      throw new Error("Cannot connect to insecure millennium server");
    }
  }
   ...
   * your machine's local time could be different from Millennium's. This gives you
   ...
   *
   * ```javascript
   * const transaction = new AiBlocksSdk.TransactionBuilder(accountId, {
   *  fee: await AiBlocksSdk.Server.fetchBaseFee(),
   *  timebounds: await AiBlocksSdk.Server.fetchTimebounds(100)
   * })
   ...
   * This is to avoid a scenario where Millennium is horking up the wrong date.
  ...
  public async fetchTimebounds(
    ...
    // MillenniumAxiosClient instead of this.ledgers so we can get at them headers
    ...
    await MillenniumAxiosClient.get(URI(this.serverURL as any).toString());
   ...
   * @see [Fee Stats](https://www.aiblocks.io/developers/millennium/reference/endpoints/fee-stats.html)
   * @returns {Promise<Millennium.FeeStatsResponse>} Promise that resolves to the fee stats returned by Millennium.
   */
  public async feeStats(): Promise<Millennium.FeeStatsResponse> {
    const cb = new CallBuilder<Millennium.FeeStatsResponse>(
   ...
   *
   * @see [Post
   * Transaction](https://www.aiblocks.io/developers/millennium/reference/endpoints/transactions-create.html)
   ...
   * [SEP0029](https://github.com/aiblocks/aiblocks-protocol/blob/master/ecosystem/sep-0029.md).
  ...
  public async submitTransaction(
  ...
  ): Promise<Millennium.SubmitTransactionResponse> {
    ...
    return MillenniumAxiosClient.post(
      URI(this.serverURL as any)
      ...
      .then((response) => {
        ...
        // TODO: fix aiblocks-base-sdk types.
            ...
            .map((result: any, i: number) => {
              ...
              const offersClaimed = offerSuccess
                ...
                .map((offerClaimed: any) => {
                  ...
                  return {
                    ...
                    assetSold,
                    amountSold: _getAmountInDelos(claimedOfferAmountSold),
                    assetBought,
                    amountBought: _getAmountInDelos(claimedOfferAmountBought),
                  };
                });
              ...
              if (
                ...
                currentOffer = {
                  ...
                  selling: {},
                  buying: {},
                  amount: _getAmountInDelos(offerXDR.amount().toString()),
                  price: {
                    n: offerXDR.price().n(),
                    d: offerXDR.price().d(),
                  },
              ...
              return {
                ...
                // this value is in sectors so divide it out
                amountBought: _getAmountInDelos(amountBought),
                amountSold: _getAmountInDelos(amountSold),
  ...
  /**
   * @returns {AccountCallBuilder} New {@link AccountCallBuilder} object configured by a current Millennium server configuration.
   */
  ...
  /**
   * @returns {ClaimableBalanceCallBuilder} New {@link ClaimableBalanceCallBuilder} object configured by a current Millennium server configuration.
   */
  ...
  /**
   * @returns {LedgerCallBuilder} New {@link LedgerCallBuilder} object configured by a current Millennium server configuration.
   */
  ...
  /**
   * @returns {TransactionCallBuilder} New {@link TransactionCallBuilder} object configured by a current Millennium server configuration.
   */
  ...
  /**
   * People on the AiBlocks network can make offers to buy or sell assets. This endpoint represents all the offers on the DEX.
   *
   ...
   * @returns {OrderbookCallBuilder} New {@link OrderbookCallBuilder} object configured by a current Millennium server configuration.
   */
  ...
  /**
   * Returns
   * @returns {TradesCallBuilder} New {@link TradesCallBuilder} object configured by a current Millennium server configuration.
   */
  ...
  /**
   * @returns {OperationCallBuilder} New {@link OperationCallBuilder} object configured by a current Millennium server configuration.
   */
  ...
  /**
   * The AiBlocks Network allows payments to be made between assets through path
   ...
   *
   * As part of the search, millennium will load a list of assets available to the
   ...
   * amount.
   *
   * If a list of assets is passed as the source, millennium will find any payment
   ...
   * @returns {StrictReceivePathCallBuilder} New {@link StrictReceivePathCallBuilder} object configured with the current Millennium server configuration.
   */
  ...
  /**
   * The AiBlocks Network allows payments to be made between assets through path payments. A strict send path payment specifies a
   ...
   * @returns {StrictSendPathCallBuilder} New {@link StrictSendPathCallBuilder} object configured with the current Millennium server configuration.
   */
   ...
   * Millennium server configuration.
   */
  ...
  /**
   * @param {string} address The AiBlocks ID that you want Friendbot to send delos to
   ...
   * Returns new {@link TradeAggregationCallBuilder} object configured with the current Millennium server configuration.
   ...
   * at the same time from Millennium.
   *
   * @see
   * [SEP0029](https://github.com/aiblocks/aiblocks-protocol/blob/master/ecosystem/sep-0029.md)
