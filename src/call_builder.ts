...
import MillenniumAxiosClient from "./millennium_axios_client";
import { ServerApi } from "./server_api";

/* tslint:disable-next-line:no-var-requires */
const version = require("../package.json").version;

// Resources which can be included in the Millennium response via the `join`
// query-param.
 ...
 * @param {string} serverUrl URL of Millennium server
 * @class CallBuilder
 */
export class CallBuilder<
  T extends
    | Millennium.FeeStatsResponse
    | Millennium.BaseResponse
    | ServerApi.CollectionPage<Millennium.BaseResponse>
> {
  protected url: URI;
   ...
   * @see [Millennium Response Format](https://www.aiblocks.io/developers/millennium/reference/responses.html)
  ...
  public stream(options: EventSourceOptions<T> = {}): () => void {
    ...
    this.url.setQuery("X-Client-Name", "js-aiblocks-sdk");
    ...
    const createEventSource = () => {
      ...
      if (es) {
        // when receiving the close message from Millennium we should
   ...
   * @see [Paging](https://www.aiblocks.io/developers/millennium/reference/paging.html)
    ...
    for (const key of Object.keys(json._links)) {
      ...
      */
      if (included && JOINABLE.indexOf(key) >= 0) {
        ...
        json[key] = this._requestFnForLink(n as Millennium.ResponseLink);
      }
    }
    return json;
  }
  ...
  private async _sendNormalRequest(initialUrl: URI) {
    ...
    return MillenniumAxiosClient.get(url.toString())
