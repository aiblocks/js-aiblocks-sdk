import { StrKey } from "aiblocks-base-sdk";
import axios from "axios";
import URI from "urijs";
import { AiBlocksTomlResolver } from "./aiblocks_toml_resolver";
 ...
 * [federation server](https://www.aiblocks.io/developers/guides/concepts/federation.html)
...
export class FederationServer {
   ...
   *
   * * For AiBlocks address (ex. `bob*aiblocks.io`) it splits AiBlocks address and then tries to find information about
   * federation server in `aiblocks.toml` file for a given domain. It returns a `Promise` which resolves if federation
   ...
   * ```
   *
   * @see <a href="https://www.aiblocks.io/developers/guides/concepts/federation.html" target="_blank">Federation doc</a>
   * @see <a href="https://www.aiblocks.io/developers/guides/concepts/aiblocks-toml.html" target="_blank">AiBlocks.toml doc</a>
   * @param {string} value AiBlocks Address (ex. `bob*aiblocks.io`)
   * @param {object} [opts] Options object
  ...
  public static async resolve(
    ...
    if (addressParts.length !== 2 || !domain) {
      return Promise.reject(new Error("Invalid AiBlocks address"));
    }
   ...
   *
   * If `aiblocks.toml` file does not exist for a given domain or it does not
   ...
   *   })
   *   .catch(error => {
   *     // aiblocks.toml does not exist or it does not contain information about federation server.
   *   });
   * ```
   * @see <a href="https://www.aiblocks.io/developers/guides/concepts/aiblocks-toml.html" target="_blank">AiBlocks.toml doc</a>
  ...
  public static async createForDomain(
    ...
    const tomlObject = await AiBlocksTomlResolver.resolve(domain, opts);
    if (!tomlObject.FEDERATION_SERVER) {
      return Promise.reject(
        new Error("aiblocks.toml does not contain FEDERATION_SERVER field"),
      );
    }
  ...
  /**
   * Get the federation record if the user was found for a given AiBlocks address
   * @see <a href="https://www.aiblocks.io/developers/guides/concepts/federation.html" target="_blank">Federation doc</a>
   * @param {string} address AiBlocks address (ex. `bob*aiblocks.io`). If `FederationServer` was instantiated with `domain` param only username (ex. `bob`) can be passed.
   * @returns {Promise} Promise that resolves to the federation record
   */
  public async resolveAddress(
    address: string,
  ): Promise<FederationServer.Record> {
    let aiblocksAddress = address;
    if (address.indexOf("*") < 0) {
      if (!this.domain) {
        return Promise.reject(
          new Error(
            "Unknown domain. Make sure `address` contains a domain (ex. `bob*aiblocks.io`) or pass `domain` parameter when instantiating the server object.",
          ),
        );
      }
      aiblocksAddress = `${address}*${this.domain}`;
    }
    const url = this.serverURL.query({ type: "name", q: aiblocksAddress });
